...
class Model:
    def __init__(self):
        ...
        self.y_div = np.array([])
        self.iter_num = 10000
        self.parameters = np.zeros(self.days + 1)
...
#        print(self.df)
        ...
        for d in date:
            if d % 100 > self.days:
                self.x = np.append(self.x, self.df.iloc[cur_row - self.days: cur_row, 3])
...
#        print(self.y)
        ...
        self.parameters[0 : self.days] = 1 / self.days
    ...
    def predict(self, n_step):
        ...
        for i in range(self.days, len(df.index)):
            pred_x = np.array(df.iloc[i - self.days : i, 3])
            for j in range(len(pred_y) - min(self.days, len(pred_y)), len(pred_y)):
                pred_x[self.days - len(pred_y) + j] = pred_y[j]
            pred_x = np.append(pred_x, 1)
            pred_y = np.append(pred_y, np.dot(pred_x, self.parameters))
        for i in range(self.days, len(df.index)):
            print(df.iloc[i, 0], pred_y[i - self.days])
        return pred_y
